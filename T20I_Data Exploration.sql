...
SELECT *
FROM T20I
WHERE ((Team1 = 'South Africa' AND Team2 = 'India') 
OR (Team2 = 'South Africa' AND Team1 = 'India'))
AND YEAR(MatchDate) = 2024

--Q2) Find the teams with the highest number of wins in 2024 and the total matches it won
...
--Q3) Rank the teams based on the total no. of wins in 2024
...
--Q4.2) Which team had the highest average winning margin (in wickets), and what was the average margin?
SELECT TOP 1 Winner, AVG(CAST(SUBSTRING(Margin, 1, CHARINDEX(' ', Margin) - 1 ) AS INT)) AS Avg_Margin
FROM T20I
...
FROM T20I T
LEFT JOIN CTE_AvgMargin A ON 1 = 1 --This condition will always be true, so table will definitely join
...
--Q9) For each team, find how many matches they pld in 2024 and their win percentage
...
SELECT m.Team, m.MatchesPlayed, ISNULL(w.Wins, 0) AS Wins,
